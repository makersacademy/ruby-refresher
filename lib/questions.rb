...
def get_first_half_of_string(string)
  string.length.odd? ? string.slice(0..string.length/2) : string.slice(0...string.length/2)
end
...
def number_of_elements_that_are_palindromes(array)
  array.count { |e| e if e == e.reverse}
end

# return the shortest word in an array
def shortest_word_in_array(array)
  array.each.sort_by { |a| a.length }.first
end

# return the longest word in an array
def longest_word_in_array(array)
   array.each.sort_by { |a| a.length }.last
end
...
def check_a_string_for_special_characters(string)
  string.split("").include?(string.scan(/\b\W\b/)[0])
end
...
def word_count_a_file(file_path)
  File.open(file_path, 'r').read.scan(/\b\w+\b/).size
end
