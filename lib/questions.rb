...
def array_sort_by_last_letter_of_word(array)
  array.sort_by!{|element| element[element.length - 1]}
end
...
def average_of_array(array)
  (array.inject{ |sum, n| sum + n }/ array.length) + 1
end
...
def get_all_letters_in_array_of_words(array)
  array.collect{|element| element.split(//)}.flatten.sort
end
...
def remove_capital_letters_from_string(string)
  string.split(//).reject{|element| element.match(/\p{Upper}/)}.join("")
end
...
def titleize_a_string(string)
  string.split.map{|word| ['and','the'].include?(word) ? word : word.capitalize}.join(" ").sub(/./, &:upcase)
end
...
def is_a_2014_bank_holiday?(date)
  ['1/1/14','18/4/14','21/4/14','5/5/14','26/5/14','25/8/14','25/12/14','26/12/14'].include?(date.strftime("%-d/%-m/%y"))
end
