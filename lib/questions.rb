...
def select_elements_starting_with_vowel(array)
  array.select { |item| ("/aeiou/").match(item[0]) }
end
...
def get_first_half_of_string(string)
  cut_length = string.length/2.round
  cut_length % 2 == 0 ? cut_length += 1 : cut_length
  return string.slice(0,(cut_length))
end
...
def is_a_2014_bank_holiday?(date)
  holidays = ["26/12/14", "25/12/14", "25/08/2014", "26/5/2014", "5/5/2014", "21/3/2014", "18/3/2014", "1/1/2015"]
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  ...
  i = 1
  year = 0
  until bday.next_year(i).to_time.wday == 5
    i += 1
    year = bday.next_year(i).to_time.year
  end
  year
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  file.each_line { |line|
    ...
    words.each { |word|
      new_word = word.tr(",","").tr(".","")
      letter_count = new_word.length
      if result.has_key?(letter_count)
        result[letter_count] = result[letter_count] + 1
      else
