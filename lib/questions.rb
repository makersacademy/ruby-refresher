...
def remove_nils_and_false_from_array(array)
  array.compact.reject { |str| str == false }
end
...
def separate_array_into_even_and_odd_numbers(array)
  evn, odd = [], []
  array.map { |x| x.even? ? (evn << x) : (odd << x) }
  combined = [evn, odd]
end
...
def get_elements_until_greater_than_five(array)
  position = array.find_index {|x| x > 5 }
  array[0...position]
end
...
def titleize_a_string(string)
  no_caps = ['a', 'and', 'the']
  str = string.split(" ").map { |word| no_caps.include?(word) ? word : word.capitalize }
  str.delete_at(0)
  str.insert(0, 'The')
  str.join(" ")
end
...
def is_a_2014_bank_holiday?(date)
  bank_holidays = ['26/12/14', '25/12/14', '25/08/14', '26/05/14', '05/05/14', '21/04/14', '18/04/14', '01/01/14']
  check_date = date.strftime('%d/%m/%y')
  bank_holidays.include?(check_date)
end
