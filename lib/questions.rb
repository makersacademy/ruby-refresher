...
def get_first_half_of_string(string)
  if string_length_is_odd?(string)
...
def string_length_is_odd?(string)
  string.length % 2 != 0
end
...
def make_numbers_negative(number)
  return number if number.to_s.chr == '-'
  number * -1
end
...
def average_of_array(array)
  float = array.inject {|sum,element| sum + element}.to_f / array.length
  float.round
end
...
def remove_capital_letters_from_string(string)
  string.split('').reject {|e| e == e.upcase unless e == ' ' }.join('')
end
...
def titleize_a_string(string)
  ...
  until array[index] == nil
    array[index].capitalize! unless array[index] == 'a' || array[index] == 'and' || array[index] == 'the'
    index += 1
  end
  array.join(' ')
end
...
def is_a_2014_bank_holiday?(date)
  date_array = [Time.new(2014, 8, 25),
    ...
    Time.new(2014, 12, 26),
    Time.new(2014, 12, 25)]
  date_array.include?(date) ? true : false
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  new_array = []
  length = 1
  until length > (array.sort_by {|element| element.length }.last).length
    new_array << [length, array.count { |element| element.length == length}]
    length += 1
  end
  new_array.to_h
end
...
def ninety_nine_bottles_of_beer
  ...
  puts "#{beer_on_the_wall} bottle of beer on the wall, #{beer_on_the_wall} bottle of beer."
