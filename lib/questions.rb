...
def shortest_word_in_array(array)
  return array.sort_by(&:length)[0]
end

# return the shortest word in an array
def longest_word_in_array(array)
  return array.sort_by(&:length)[-1]
end
...
# so [10, 15, 25] should return 17
  #need to have a look at this agin........
...
def round_up_number(float)
  return float.to_f.ceil
end

# round down a float up and convert it to an Integer,
# so 9.52 becomes 9
def round_down_number(float)
  return float.to_f.floor
end
...
def check_a_string_for_special_characters(string)
  if string.count("^a-zA-Z0-9").zero?
    return false
  else
    return true
  end
...
def is_a_2014_bank_holiday?(date)
  ...
  bh = ["2014/01/01", "2014/04/18", "2014/04/21", "2014/05/05", "2014/05/26", "2014/08/25", "2014/12/25", "2014/12/26"]
  bh.include?(date)
  #return date
...
def count_words_of_each_length_in_a_file(file_path)
  length_of_word = Hash.new(0)
  array = []
    array = File.open(file_path, 'r') do |y|
      y.readline.gsub(/[^a-z ]/i,"").split(" ")
      end
    array.each do |x|
      length_of_word[x.length]+=1
    end
