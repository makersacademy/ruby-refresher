...
def select_elements_starting_with_vowel(array)
   array.select{|item| item[0].match(/[aeiou]/)};
end
...
def get_first_half_of_string(string)
  string.slice(0..(string.length.to_f/2).ceil-1)
end
...
def separate_array_into_even_and_odd_numbers(array)
  [array.select{|item|item.even?}, array.select{|item|item.odd?}]
end
...
def add_together_keys_and_values(hash)
  (hash.keys+hash.values).inject(0){|sum, value|sum+=value}
end
...
def titleize_a_string(string)
  array = string.capitalize.split(' ').each{|item|item.capitalize! unless item=="the"||item=="a"||item=="and"}
  outputStr = array.join(" ")
end
