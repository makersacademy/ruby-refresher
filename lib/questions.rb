...
def average_of_array(array)
	(array.inject(0, :+).to_f / array.size).ceil
end
...
def titleize_a_string(string)
	function_words = ["a", "and", "the"]
	string.capitalize.split(" ").each { | word | 
		word.capitalize! unless function_words.include?(word)
		}.join(" ")
end
...
def is_a_2014_bank_holiday?(date)
	bank_holidays = ["26/12/2014", "25/12/2014", "25/08/2014",
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
	while birthday.strftime("%A") != "Friday"
		...
		year = birthday.strftime("%Y").to_i + 1
