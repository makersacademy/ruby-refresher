...
def separate_array_into_even_and_odd_numbers(array)
	result = [[],[]]
	array.each { |element| element.even? ? result[0].push(element) : result[1].push(element) }
	return result
end
...
def average_of_array(array)
	sum = 0.0
	array.each{ |element| sum += element.to_f }
	return (sum / array.length).round
end
...
def count_words_of_each_length_in_a_file(file_path)
	h = {}
	words = File.read(file_path).gsub(/[[:punct:]]/, '').split
	words.each { |word| h.has_key?(word.length) ? h[word.length] += 1 : h[word.length] = 1 }		
	h
end
...
def ninety_nine_bottles_of_beer
	...
	puts "#{beer_count} bottle of beer on the wall, #{beer_count} bottle of beer"
