...
def select_elements_starting_with_vowel(array)
  array.select { |word| word[0].downcase[/([aeiou])/] }
end
...
def remove_nils_and_false_from_array(array)
  remove_nils_from_array(array).reject { |value| value == false }
end
...
# count the numbers of elements in an array which are palindromes
# a palindrome is a word that's the same backwards as forward
# e.g. 'bob'. So in the array ['bob', 'radar', 'eat'], there
# are 2 palindromes (bob and radar), so the method should return 2
def number_of_elements_that_are_palindromes(array)
  (array & reverse_every_element_in_array(array)).length
end
