...
def select_elements_starting_with_a(array)
  array.select { |person| person[0] == 'a' }
end
...
def get_first_half_of_string(string)
  first_half = (string.length / 2.to_f).ceil
  return string.slice(0, first_half)
end
...
def get_elements_until_greater_than_five(array)
  array.slice_before { |element| element >5 }.to_a[0]
end
...
def titleize_a_string(string)
  ...
  string.split(" ").each_with_index do |element, index|
    if !(exceptions.include? element) || (index == 0)
      new_string.push element.capitalize
    else
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  while !birthday == birthday.friday? do
...
def fizzbuzz_without_modulo
  1.upto(100) do |i|
    return "FizzBuzz" if (i / 15) * 15 == i
    return "Buzz" if (i / 5) * 5 == i
    return "Fizz" if (i / 3) * 3 == i
    return i
  end
end
...
# at the end.
# (there's no RSpec test for this one)
def ninety_nine_bottles_of_beer(quantity, new_quantity)
  ...
  ninety_nine_bottles_of_beer(quantity-1, new_quantity)
  end
end
...
ninety_nine_bottles_of_beer(99, 99)
