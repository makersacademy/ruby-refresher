...
def remove_nils_from_array(array)
  array.reject {|str| str.to_s.empty?}
end

# remove instances of nil AND false from an array
def remove_nils_and_false_from_array(array)
  array.reject {|str| str == false || str == nil}
end
...
def separate_array_into_even_and_odd_numbers(array)
  ...
  array.map {|x| x % 2 == 0 ? even_array << x : odd_array << x }
...
def number_of_elements_that_are_palindromes(array)
  count = 0
  array.each {|x| count += 1 if x == x.reverse}
  count
end
...
def average_of_array(array)
  to_return = array.inject{ |sum, el| sum + el }.to_f / array.size
  to_return.round
end
...
def convert_array_to_a_hash(array)
  hash = {}
  array.each_slice(2) {|a, b| hash[a] = b }
  hash
end
...
def get_all_letters_in_array_of_words(array)
  to_return = []
  array.map {|x| x.split("").each {|x| to_return << x }}
  to_return.sort
end
...
def get_domain_name_from_email_address(email)
   email.gsub(".com", "").slice(email.index("m")..-1)
end
