...
def select_elements_starting_with_vowel(array)
  array.select!{|element| element.scan(/[aeiou]/).include?(element[0])}
...
def all_elements_except_first_3(array)
  array.select!{|element| array[element-1] > 3}
  # just because the array is made of numbers :)
end
...
def get_first_half_of_string(string)
  return string[0..string.length/2] if string.length % 2 != 0
...
def separate_array_into_even_and_odd_numbers(array)
  ...
  array.each do |element|
    odd.push(element) if element % 2 != 0
    even.push(element) if element % 2 == 0
  end
  result << even
  result << odd
  result
...
def shortest_word_in_array(array)
  array.sort_by{|element| element.length}.shift
end
...
def average_of_array(array)
  (array.inject{|sum,elem| sum + elem}/array.length.to_f).round
end
...
def get_elements_until_greater_than_five(array)
  result = []
  array.each{|element| break if element > 5; result << element }
  result
end
...
def get_all_letters_in_array_of_words(array)
  array.map{|element| element.chars}.flatten.uniq.sort
end
...
def add_together_keys_and_values(hash)
  sum = 0
  hash.each_pair{|key,value| sum += key + value}
  sum
end
