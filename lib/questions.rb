...
def average_of_array(array)
  (array.inject { |sum, number| sum + number }.to_f / array.length).ceil
end
...
def add_together_keys_and_values(hash)
  hash.flatten.inject { |sum, number| sum + number }
end
...
def fizzbuzz_without_modulo
  ...
  until number == 21
    case
    when (number - 15 * (number / 15).floor).zero? then puts "Fizzbuzz"
    when (number - 3 * (number / 3).floor).zero? then puts "Fizz"
    when (number - 5 * (number / 5).floor).zero? then puts "Buzz"
    else puts number
    end
    number += 1
  end
