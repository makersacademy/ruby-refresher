...
def remove_nils_and_false_from_array(array)
    array.delete_if {|element| !(!!element == true)}
end
...
def get_first_half_of_string(string)
  new_length = (string.length/2.to_f).ceil
  string = string[0..(new_length - 1)]
end
...
def separate_array_into_even_and_odd_numbers(array)
  separated = [[], []]
  array.each {|number| number % 2 == 0 ? separated[0] << number : separated[1] << number}
  separated
end
...
def number_of_elements_that_are_palindromes(array)
  array.select! { |element| element.reverse == element }
  array.count
end
...
def average_of_array(array)
  divisor = array.count
  result = ((array.inject { |a, b| (a + b)})/divisor.to_f).round
...
def titleize_a_string(string)
  ...
  n = 1
  until array[n] == nil
    array[n].capitalize! unless array[n] == "a" || array[n] == "and" || array[n] == "the"
    n += 1
  end
  array.join(" ")
end
...
def word_count_a_file(file_path)
  File.open(file_path, "r") {|file|  (file.read.split(" ")).count }
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  ...
  next_friday_birthday = nil
  while next_friday_birthday == nil
...
def ninety_nine_bottles_of_beer
  # No time now, but I have done this one with Chris Pine
end
