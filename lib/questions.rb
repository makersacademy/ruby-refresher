...
def select_elements_starting_with_vowel(array)
  array.select {|element| element =~ /\A[aeiou]/}
end
...
def remove_nils_and_false_from_array(array)
  array.select {|element| element != false && element != nil}
end
...
def separate_array_into_even_and_odd_numbers(array)
  ...
  odd = array.select {|element| element % 2 != 0 }
  [even, odd]
end
...
def get_all_letters_in_array_of_words(array)
  array.map {|e| e.chars}.flatten.sort
end
...
def swap_keys_and_values_in_a_hash(hash)
  hash.to_a.map{|e| e.reverse}.to_h
end
...
def titleize_a_string(string)
  words_to_ignore = ['the', 'a', 'and']
  new_string = string.split(" ").each do |i|
...
def word_count_a_file(file_path)
  ...
  file.each_line {|line| return line.split.count}
end
...
def is_a_2014_bank_holiday?(date)
  bank_holidays = ['26/12/2014',
     '25/12/2014', '25/08/2014',
     '26/05/2014', '05/05/2014',
     '21/04/2014', '18/04/2014',
     '01/01/2014']
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  # end
  # counts
  #reads the files text and returns string of text. Scan then returns all the words
  words = IO.read(file_path).scan(/\w+/)
  count = Hash.new(0)
  words.map { |word| count[word.length] += 1 }
  count
end
...
def fizzbuzz_without_modulo
  fizz = 0; buzz = 0; fizzbuzz = 0; counter = 0

  until counter == 100
    fizz += 1; buzz += 1; fizzbuzz += 1; counter += 1
    if fizzbuzz == 15
      puts "fizzbuzz"
      fizzbuzz = 0
      fizz = 0
