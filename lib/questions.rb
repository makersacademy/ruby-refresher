...
def select_elements_starting_with_vowel(array)
    array.select{|element| element if element[0].count("aeiou") > 0 }
end
...
def get_first_half_of_string(string)
  string.length % 2 > 0 ? middle = (string.length / 2) + 1 : middle = string.length / 2
  string.chars.each_slice(middle).map(&:join)[0]
end
...
def shortest_word_in_array(array)
  array.min{|elem1, elem2| elem1.size <=> elem2.size }
end

# return the shortest word in an array
def longest_word_in_array(array)
  array.max{|elem1, elem2| elem1.length <=> elem2.length}
end
...
def average_of_array(array)
  ((array.inject(0){|sum,element| sum + element}.to_f) / array.length).to_f.ceil
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  words.each{|word| output_hash[word.length] += 1}
  output_hash
end
