...
def average_of_array(array)
  (array.reduce(:+) * 1.0 / array.count).round
end
...
def get_all_letters_in_array_of_words(array)
  array.join.each_char.to_a.uniq.sort
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  7.times do |i|
    year = birthday.year + 1 + i
    return year if Date.new(year, birthday.mon, birthday.mday).strftime('%a') === 'Fri'
  end
end
...
def count_words_of_each_length_in_a_file(file_path)
  count_of_length = Hash.new(0)
  File.open("./#{file_path}", "r") do |file|
    file.readlines.each do |line|
      line.split(/[ .,]/).each do |word|
        count_of_length[word.length] += 1 unless word.empty?
      end
    end
  end
  count_of_length
