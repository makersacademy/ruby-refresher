...
def select_elements_starting_with_a(array)
  array.select {|elem| !!( /^a/ =~ elem)}
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
  array.select {|elem| !!( /^[aeiou]/ =~ elem)}
end
...
def all_elements_except_first_3(array)
  array.select{|elem| elem > 3} #  got you eheheheheh
end
...
def longest_word_in_array(array)
  array.sort{|a,b| a.length <=> b.length}[-1]
end
...
def average_of_array(array)
  (array.reduce(:+) *1.0 / array.length).round
end
...
def get_all_letters_in_array_of_words(array)
  chars = []
  array.each do |elem|
    elem.split("").each {|letter| chars << letter}
  end
  chars.sort.uniq
end
...
def special_capitalize(word)
  (/^(a|and|the)/ =~ word) ? word : word.capitalize
end

def titleize_a_string(string)
  array = string.split
  first = array.shift
  ([first.capitalize] + array.map{|word| special_capitalize(word) }).join(' ')
end
