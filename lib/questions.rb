...
def get_first_half_of_string(string)
  string[0..(string.length.to_f/2).round-1]
end
...
def separate_array_into_even_and_odd_numbers(array)
  [array.select{|a| a.even?}, array.select{|a| !a.even?}]
end
...
def number_of_elements_that_are_palindromes(array)
  array.select{|a| a.reverse == a}.count
end
...
def titleize_a_string(string)
  special = ['and','the','a']
  string.capitalize.split.collect{|word| (word == special[0] || word == special[1] || word == special[2])? word : word.capitalize }.join(" ")
end
...
def fizzbuzz_without_modulo
  ...
  while i <= 100 do
    if modulo(i,15).zero?
      puts "#{i} 'fizzbuzz'"
    elsif modulo(i,3).zero?
      puts "#{i} 'fizz'"
    elsif modulo(i,5).zero?
      puts "#{i} 'buzz'"
    else
      puts "#{i}"
    end
...
def ninety_nine_bottles_of_beer
  ...
  while i > 0
    ...
    print_bottles = 'bottle' if i == 1
  	puts "#{english_number(i)} #{print_bottles} of beer on the wall, #{english_number(i)} #{print_bottles} of beer."
  	i -=1
    print_bottles = 'bottle' if i == 1
    if i != 0
      puts "Take one down and pass it around, #{english_number(i)} #{print_bottles} of beer on the wall.\n\n"
    else
