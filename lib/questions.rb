...
def longest_word_in_array(array)
  array.sort_by! { |element| element.length }.pop
end
...
def average_of_array(array)
  (array.inject(0.0, :+) / array.size).ceil
end
...
def get_all_letters_in_array_of_words(array)
  array.flat_map { |element| element.chars }.sort
end
...
def titleize_a_string(string)
  ignore = ["the", "and", "a"]
  string.split[0].capitalize + " " + string.split[1..string.length].map do |word|
...
def is_a_3_dot_range?(range)
  range.last == range.size ? false : true
end
...
def is_a_2014_bank_holiday?(date)
  ...
  bank_hols_2014 = ['01/01/14', '18/04/14', '21/04/14',
                    '05/05/14', '26/05/14', '25/08/14',
                    '25/12/14', '26/12/14']
  bank_hols_2014.include? date
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  file = File.open(file_path, "r")
  file.each_line { |line|
    line.scan(/[\w\']+/).each { |word|
      freq.has_key?(word.length) ? freq[word.length] += 1 : freq[word.length] = 1
    }
  }
  freq
end
