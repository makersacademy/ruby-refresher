...
def every_possible_pairing_of_students(array)
  (array.repeated_combination(2).to_a).reject{|names| (names[0] == names[1]) }
end
...
def array_sort_by_last_letter_of_word(array)
  array.map{|animal| animal.reverse}.sort.map{|revanimal| revanimal.reverse}
end
...
def separate_array_into_even_and_odd_numbers(array)
  even = []
  odd = []
  array.each{|number| number.even? ? even<<number : odd<<number}
  [even, odd]
end
...
def number_of_elements_that_are_palindromes(array)
  array.reject{|name| name != name.reverse}.length
end
...
def is_a_3_dot_range?(range)
  Range.new(range.begin, range.end) == range ? false: true
end
