...
def get_first_half_of_string(string)
  if string.length % 2 != 0
    characters = (string.length + 1)/2
  else
    characters = (string.length)/2
  end
  string[0..characters - 1]
end
...
def add_together_keys_and_values(hash)
  array_of_hash = hash.flatten
  array_of_hash.reduce{|sum, num| sum += num}
end
...
def titleize_a_string(string)
  string.capitalize!
  words_to_keep_lowercase = ['a', 'and', 'the']
  title = string.split(" ").map do |word|
    if words_to_keep_lowercase.include?(word)
      word
    else
      word.capitalize
    end
...
def is_a_3_dot_range?(range)
  dots = range.to_s.split("").count(".")
  return true if dots == 3
  false
end
