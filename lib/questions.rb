...
def select_elements_starting_with_a(array)
  array.select { |word| word.downcase[0] == 'a'}
end
...
def separate_array_into_even_and_odd_numbers(array)
  [array.select { |num| num % 2 == 0 }, array.select { |num| num % 2 != 0 }]
end
...
def check_a_string_for_special_characters(string)
  special = false
  string.each_char { |char| special = true unless ('0'..'9').to_a.include?(char) || ('a'..'z').to_a.include?(char.downcase) }
  special
end
...
def word_count_a_file(file_path)
  ...
  sum = 0
  file.each_line { |line| sum += line.split(' ').length }
  sum
end
