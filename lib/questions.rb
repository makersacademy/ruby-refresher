...
def remove_nils_and_false_from_array(array)
   array.compact.select{|item| item != false }
end
...
def number_of_elements_that_are_palindromes(array)
  array.count{|elem| elem == elem.reverse}
end

# return the shortest word in an array
def shortest_word_in_array(array)
  array.sort_by{|elem| elem.length}.first
end

# return the shortest word in an array
def longest_word_in_array(array)
  array.sort_by{|elem| elem.length}.last
end
...
def convert_array_to_a_hash(array)
  output = Hash[array.each_slice(2).to_a]
end
...
def get_all_letters_in_array_of_words(array)
  output = []
  array.each{|elem| output.push(elem.split(""))}
  output.flatten.sort
end
...
def titleize_a_string(string)
        string.capitalize!
        words_not_to_capitalize = ["and", "the"]
...
def is_a_2014_bank_holiday?(date)
  bank_hols = ["2014/12/26","2014/12/25","2014/08/25","2014/05/26","2014/05/05","2014/04/21","2014/04/18","2014/01/01"]
  bank_hols.include?(date.strftime("%Y/%m/%d")) ? true : false
end
...
def count_words_of_each_length_in_a_file(file_path)
  data = File.open(file_path).read.scan(/[\w']+/)
  totals = Hash.new(0)
  data.each { |word|
