...
def separate_array_into_even_and_odd_numbers(array)
  ...
  even_array = array.select{|number| number.even?}
  new_array = [even_array] + [odd_array]
end
...
def average_of_array(array)
  answer = (array.inject(:+).to_f) / (array.length.to_f)
  answer.ceil
end
...
def add_together_keys_and_values(hash)
  hash_keys_total = hash.keys.inject(:+)
  hash_values_total = hash.values.inject(:+)
  hash_keys_total + hash_values_total
end
...
def titleize_a_string(string)
  wordsnottocap = ["and", "the", "a"]
  new_string = string.capitalize!
  new_string = string.split(" ").map {|word|
  if wordsnottocap.include?(word)
    word
  else
    word.capitalize
  end
