...
def select_elements_starting_with_vowel(array)
  array.select { |item| "aeiouy".include? item[0] }
end
...
def all_elements_except_first_3(array)
  start = 3
  reduced_length = array.length - start
  array.slice(start, reduced_length)
end
...
def separate_array_into_even_and_odd_numbers(array)
  evens = array.select {|item| item.even? }
  odds = array.select {|item| item.odd? }
  [evens, odds]
end
...
def get_elements_until_greater_than_five(array)
  index = array.find_index { |item| item > 5 }
  array.slice(0, index)
end
...
def get_all_letters_in_array_of_words(array)
  array.collect { |item| item.chars }.flatten.sort
end
...
def remove_capital_letters_from_string(string)
  string.chars.select { |letter| letter == letter.downcase }.join
end

# round up a float up and convert it to an Integer,
# so 3.214 becomes 4
def round_up_number(float)
  float - float.to_i < 0.5 ? (float + 1).round : float.round
end
...
def titleize_a_string(string)
  ...
  string.capitalize.split(" ").collect {|word| exceptions.include?(word) ? word : word.capitalize }.join(" ")
end
...
def count_words_of_each_length_in_a_file(file_path)
  words = File.open(file_path)
  words = words.read.gsub(/[^a-z0-9\s]/i, '').split(" ")
  words.sort_by! { |word| word.length }
  word_length_histogram = Hash.new(0)
  words.each { |word| word_length_histogram[word.length] += 1 }
  return word_length_histogram
end
