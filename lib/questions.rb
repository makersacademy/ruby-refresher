...
def every_possible_pairing_of_students(array)
  array.repeated_combination(2).to_a.reject! { |pair| pair[0] == pair[1] }
end
...
def get_first_half_of_string(string)
  string.split("").take((string.length + 1) / 2).join
end
...
def convert_array_to_a_hash(array)
  result = Hash.new
  array.each_slice(2) { |a| result[a.first] = a.last }
  result
end
...
def titleize_a_string(string)
  ignore_these_words = ['a', 'and', 'the']
  words_array = string.split(' ')
  words_array.each do |word|
    word.capitalize! unless ignore_these_words.include?(word)
  end
...
def is_a_3_dot_range?(range)
  !range.to_s.include?(range.sort.last.to_s)
end
