...
def is_a_2014_bank_holiday?(date)
  bank_holidays_2014 = [Time.parse('01/01/2014'), Time.parse('18/04/2015'), Time.parse('21/04/2014'),
                        Time.parse('05/05/2014'), Time.parse('26/05/2014'), Time.parse('25/08/2014'),
                        Time.parse('25/12/2014') , Time.parse('26/12/2014')]
...
def count_words_of_each_length_in_a_file(file_path)
  hash = {}
  array = File.open(file_path).read.downcase.gsub(/[^a-z\s]/, '').split.map{|e| e.length}.sort
  array.each do |e|
...
def fizzbuzz_without_modulo
  array = (1..100).to_a
  for i in 1..100/3 do array[i] = 'fizz' end
  for j in 1..100/5 do array[j] = 'buzz' end
  for k in 1..100/15 do array[k] = 'fizzbuzz' end
  return array
end
