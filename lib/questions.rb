...
def get_first_half_of_string(string)
    h = string.length / 2
    string.length.odd? ? string[0..h] : string[0..h-1]
end
...
def titleize_a_string(string)
  lower = %w{a and the}
  string.split.each_with_index.map { |word, index| lower.include?(word) && index > 0 ? word : word.capitalize }.join(" ")
end
...
def check_a_string_for_special_characters(string)
  !string.gsub(/[a-zA-Z0-9]/, "").split("").empty?
end
...
def count_words_of_each_length_in_a_file(file_path)
  IO.read(file_path).gsub(/[^a-zA-Z ]/, "").split.map{ |x| x.size }.each_with_object(Hash.new(0)) { |letter, hash|
hash[letter] += 1}
end
