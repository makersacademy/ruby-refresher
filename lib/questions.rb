...
def get_first_half_of_string(string)
  string.slice(0..(string.length.to_f/2 - 1).ceil)
end
...
def separate_array_into_even_and_odd_numbers(array)
  [array.select { |num| num % 2 == 0 }, array.select { |num| num % 2 == 1 }]
end
...
def add_together_keys_and_values(hash)
  memo = 0
  hash.each { |key, value| memo += key+value}
  memo
end
...
def titleize_a_string(string)
  ...
  result = string.split.map { |word| no_caps.include?(word) ? word : word.capitalize }.join(" ")
end
