...
def select_elements_starting_with_a(array)
  array.select {|str| str.chars.first == 'a' }
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
  array.select {|str| str =~ /\b[aeiou]/ }
end
...
def titleize_a_string(string)
  exclude = ['a', 'the', 'and']
  p string.split.map { |word| word.capitalize unless exclude.include?(word) }.join(' ')
end
...
def check_a_string_for_special_characters(string)
   /[^A-Za-z0-9\s]/.match(string) ? true : false
end
...
def is_a_2014_bank_holiday?(date)
  ...
  Time.new(2014, 4, 21),
  Time.new(2014, 4, 18),
  Time.new(2014, 1, 1)].include?(date)
end
...
require 'Date'
def your_birthday_is_on_a_friday_in_the_year(birthday = Date.new)
  i = 0
  until birthday.friday?
    p birthday.next_year(i)
    i += 1
  end
end
...
def fizzbuzz_without_modulo(number)
  a = { 15 => 'fizzbuzz', 3 => 'fizz', 5 => 'buzz' }
  number = number.to_f
  a.each_pair do |key, value|
    if number - key*(number/key).floor == 0
      return value
    else return number
    end
  end
end
fizzbuzz_without_modulo(5)
