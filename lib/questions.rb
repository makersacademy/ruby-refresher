...
def select_elements_starting_with_vowel(array)
  array.select {|element| (element[0] =~ /[aeiou]/) == 0}
end
...
def get_elements_until_greater_than_five(array)
  array.each_with_index {|element,index| return array[0...index] if element > 5}
  return array
end
...
def convert_array_to_a_hash(array)
  ...
  array.length.times do |index|
    ret[array[index]] = array[index+1] if index.even?
  end
  ret
end
...
def titleize_a_string(string)
  string.split.map.with_index do |word, index|
    if index == 0 || (!["a","and","the"].include? word)
      word.capitalize
    else
      word
    end
...
def count_words_of_each_length_in_a_file(file_path)
  ret = Hash.new(0)
  file = File.open(file_path, "rb").read.gsub(/[,\.]/,"")
...
def fizzbuzz_without_modulo
  1.upto(100) do |number|
    ret_string = "#{number}: "
    ret_string += 'fizz' if fizzbuzz_helper number, 3
    ret_string += 'buzz' if fizzbuzz_helper number, 5
    puts ret_string
  end
end

def fizzbuzz_helper number, div
  res = number / div
  res * div == number
end
