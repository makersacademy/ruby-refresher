...
def select_elements_starting_with_a(array)
  array.select {|el| el if el[0] == "a"}
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
  array.select {|el| el if "aeiou".include?(el[0]) }
end
...
def remove_nils_and_false_from_array(array)
  array.reject {|el| el == nil || el == false}
...
def get_first_half_of_string(string)
  size = string.size
  size.even? ? string.slice(0, size/2) : string.slice(0, size/2 + 1)
...
def separate_array_into_even_and_odd_numbers(array)
  array.group_by {|el| el % 2 == 0}.values.reverse
end
...
def number_of_elements_that_are_palindromes(array)
  array.select {|el| el if el == el.reverse}.size
end
...
def get_elements_until_greater_than_five(array)
  number = array.select {|el| el if el > 5}.first
  array[0, array.index(number)]
end
...
def swap_keys_and_values_in_a_hash(hash)
  Hash[hash.to_a.map {|el| el.reverse}]
...
def word_count_a_file(file_path)
  File.readlines(file_path).join.split(" ").size
...
def is_a_2014_bank_holiday?(date)
  require 'open-uri'
  open("https://www.gov.uk/bank-holidays") do |f|
    each_year_holidays = f.read.split("table")
    holidays_2014 = each_year_holidays.select {|table| table if table.include?("2014")}
    holidays_2014.join.include?(date.strftime("%d %B"))
  end
end
...
def ninety_nine_bottles_of_beer
  ...
  while n > 0
  	if n == 2
  	  ...
  	  puts "#{n} bottle of beer on the wall, 1 bottle of beer.\nTake one down and pass it around, no more bottles of beer on the wall."
  	else
